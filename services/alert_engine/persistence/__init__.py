...
class AlertWriter:
    ...
    def buffer_alert(self, alert_dict: Dict, enriched: Optional[Dict] = None):
        if len(self._buffer) >= self.MAX_BUFFER:
            ...
            logger.warning(f"Buffer overflow — dropped {drop} oldest alerts")
        self._buffer.append({"alert": alert_dict, "enriched": enriched})

    async def run(self):
        self._running = True
        logger.info("AlertWriter started (COPY protocol, flush every %ds)", self.FLUSH_INTERVAL)
        while self._running:
    ...
    async def stop(self):
        ...
        if self._buffer:
            await self._flush()
        logger.info("AlertWriter stopped (total persisted: %d)", self._total_flushed)
    ...
    async def _flush(self):
        ...
        try:
            ...
            logger.error("COPY flush failed: %s", str(e)[:250])
