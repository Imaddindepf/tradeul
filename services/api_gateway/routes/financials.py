...
from services.edgar import get_edgar_service, EdgarService
...
async def clear_segment_cache(symbol: str):
    ...
    service = get_edgar_service()
    await service.invalidate_cache(symbol.upper())
...
async def get_company_info(symbol: str):
    ...
    try:
        ...
        result = {
            ...
            "is_insurance": company.is_insurance,
...
# ============================================================================
# EDGAR ENRICHMENT - Campos adicionales que SEC-API no extrae
# ============================================================================
...
async def get_enrichment(
    ...
    """
    Obtener campos adicionales que SEC-API no extrae.
    
    Usa edgartools para extraer:
    - Investment income
    ...
    try:
        service = get_edgar_service()
        enrichment = await service.get_enrichment(symbol, max_years=years)
        
        if not enrichment.fields:
            raise HTTPException(
                status_code=404,
                detail=f"No enrichment data for {symbol}"
            )
        ...
        logger.error("enrichment_error", symbol=symbol, error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
