...
import { EVENT_WIRE_PAIRS } from '@/lib/filter-catalog.generated';
...
export function EventTableContent({ categoryId, categoryName, eventTypes: initialEventTypes, strategyId, defaultFilters }: EventTableContentProps) {
  ...
  useEffect(() => {
    ...
    // Todos los filtros numéricos — generados desde shared/config/filter_catalog.json.
    // Orden de precedencia: override del usuario → default de la categoría → omitir.
    for (const [minK, maxK, wireMin, wireMax] of EVENT_WIRE_PAIRS) {
      setF(wireMin, (filters as any)[minK] ?? (df as any)?.[minK]);
      setF(wireMax, (filters as any)[maxK] ?? (df as any)?.[maxK]);
    }
    // String filters
  ...
  useEffect(() => {
    ...
    filterDebounceRef.current = setTimeout(() => {
      ...
      // Todos los filtros numéricos del catálogo generado (null = limpiar filtro)
      for (const [minK, maxK, wireMin, wireMax] of EVENT_WIRE_PAIRS) {
        uF(wireMin, (f as any)[minK] ?? (df as any)?.[minK]);
        uF(wireMax, (f as any)[maxK] ?? (df as any)?.[maxK]);
      }
