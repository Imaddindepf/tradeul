  ...
  return (
    <div className={`h-full flex flex-col bg-surface ${fontClass}`}>
          ...
          {data && (
            <div className="flex items-center gap-3 text-[10px] text-muted-fg">
              <span>
                ...
                <span className="text-foreground font-medium">{data.total_tickers.toLocaleString()}</span>
              </span>
              <span>
                <span className="text-muted-fg">Cap:</span>{' '}
                <span className="text-foreground font-medium">{formatMarketCap(data.total_market_cap)}</span>
              </span>
              <span>
                <span className="text-muted-fg">Avg:</span>{' '}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-[10px]">
            ...
            {session && (
              <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${
                session.current_session === 'MARKET_OPEN' 
                  ...
                  : 'bg-surface-inset text-foreground/80'
              }`}>
            ...
            {data && !data.is_realtime && (
              <span 
                className="flex items-center gap-1 px-1.5 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded text-[9px]"
          ...
          <button
            onClick={refresh}
            disabled={isLoading}
            className="p-1 text-muted-fg hover:text-foreground/80 hover:bg-surface-hover rounded disabled:opacity-50"
            title="Refresh"
          >
      ...
      <div ref={containerRef} className="flex-1 relative overflow-hidden bg-surface">
        ...
        {isUpdating && data && (
          <div className="absolute top-2 right-2 z-10 flex items-center gap-1.5 px-2 py-1 bg-surface/90 border border-border rounded shadow-sm">
            <div className="w-2 h-2 border border-blue-500 border-t-transparent rounded-full animate-spin" />
            <span className="text-[9px] text-muted-fg">Updating...</span>
          </div>
        )}
        ...
        {error && !data && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-fg text-[10px] mb-3">{error}</p>
              <button
                onClick={refresh}
                className="px-3 py-1 bg-primary text-white text-[10px] rounded hover:bg-primary-hover"
              >
                Retry
              </button>
            </div>
        ...
        {isLoading && !data && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-fg text-[11px]">Loading market data...</p>
            </div>
          </div>
        )}
        ...
        {data && (!data.sectors || data.sectors.length === 0) && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <p className="text-muted-fg text-[11px]">No data available for current filters</p>
              <p className="text-muted-fg text-[10px] mt-1">Try adjusting the market cap or sector filters</p>
            </div>
          </div>
        )}
      </div>
