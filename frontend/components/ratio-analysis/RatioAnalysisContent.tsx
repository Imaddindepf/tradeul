            ...
            <div className="flex items-center justify-between px-3 py-1 border-b border-border-subtle">
                <div className="flex items-center gap-4 text-[10px]">
                    <span className="flex items-center gap-1.5">
                        ...
                        <span className="text-muted-fg text-[9px]">(buy)</span>
                        <span className="text-muted-fg">${data.prices.y.latest}</span>
                    </span>
                    <span className="flex items-center gap-1.5">
                        <span className="w-3 h-0.5 bg-cyan-500 rounded"></span>
                        <span className="text-cyan-600 font-medium">{data.symbols.x}</span>
                        <span className="text-muted-fg text-[9px]">(sell)</span>
                        <span className="text-muted-fg">${data.prices.x.latest}</span>
                    </span>
                </div>
            ...
            <ResponsiveContainer width="100%" height="85%">
                <ComposedChart data={normalizedData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                        ...
                        labelStyle={{ color: 'var(--color-muted-fg)', marginBottom: '4px' }}
                    ...
                    />
                    <ReferenceLine y={0} stroke="var(--color-muted-fg)" strokeDasharray="3 3" />
                    <Line
                        type="monotone"
                        dataKey="yNorm"
                        stroke="#f43f5e"
        ...
        <div className="h-[140px]">
            <div className="flex items-center justify-between px-3 py-1 border-b border-border-subtle">
                <div className="flex items-center gap-2 text-[10px]">
                    <span className="text-foreground/80 font-medium">{data.symbols.y}/{data.symbols.x}</span>
        ...
        <div className="h-[140px]">
            ...
            <ResponsiveContainer width="100%" height="85%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    />
                    <ReferenceLine y={0.5} stroke="var(--color-muted-fg)" strokeDasharray="3 3" />
                    <Area
                        type="monotone"
                        dataKey="corr"
                        stroke="#8b5cf6"
...
function RegressionPanel({ data }: { data: RatioAnalysisData }) {
    ...
    if (!reg) {
        return (
            <div className="h-[180px] flex items-center justify-center text-muted-fg text-sm">
    ...
    return (
        <div className="h-[200px] flex">
                ...
                <ResponsiveContainer width="100%" height="85%">
                    <ScatterChart margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                        ...
                        <XAxis
                            ...
                            tickLine={false}
                            axisLine={{ stroke: 'var(--color-border)' }}
                            label={{ value: `${data.symbols.x} Returns (%)`, position: 'bottom', fontSize: 9, fill: 'var(--color-muted-fg)', offset: 5 }}
                            domain={['auto', 'auto']}
                        />
                        <YAxis
                            type="number"
                            ...
                            tickLine={false}
                            axisLine={false}
                            label={{ value: `${data.symbols.y} Returns (%)`, angle: -90, position: 'insideLeft', fontSize: 9, fill: 'var(--color-muted-fg)' }}
                            width={40}
                            domain={['auto', 'auto']}
                        />
                        <Tooltip
            ...
            {/* Stats Table */}
            <div className="w-[180px] border-l border-border bg-surface-hover">
                <div className="px-3 py-1 border-b border-border bg-surface-inset">
                    <span className="text-[10px] font-semibold text-foreground/80">Regression Stats</span>
                </div>
...
function StatRow({ label, value, muted = false, color }: { label: string; value: string; muted?: boolean; color?: string }) {
    return (
        <div className="flex items-center justify-between">
            <span className={`text-[10px] ${muted ? 'text-muted-fg' : 'text-foreground/80'}`}>{label}</span>
            <span className={`text-[10px] font-mono ${color || (muted ? 'text-muted-fg' : 'text-foreground font-semibold')}`}>{value}</span>
        </div>
    );
}
...
function SignalPanel({ data }: { data: RatioAnalysisData }) {
    ...
    const getSignalBg = (signal: string) => {
        if (signal.includes('LONG')) return 'bg-emerald-500/10 border-emerald-500/30';
        if (signal.includes('SHORT')) return 'bg-rose-500/10 border-rose-500/30';
    ...
    return (
        <div className={`px-3 py-2 border-b ${getSignalBg(summary.signal)}`}>
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-center">
                        <div className={`text-lg font-bold font-mono ${
                            ...
                            Math.abs(zscore.current) >= 1 ? 'text-amber-600' : 'text-foreground/80'
                        }`}>
                            {zscore.current >= 0 ? '+' : ''}{zscore.current.toFixed(2)}
                        </div>
                        <div className="text-[9px] text-muted-fg">Z-Score</div>
                    </div>
                    
                    {/* Visual gauge */}
                    <div className="w-24 h-2 bg-muted rounded-full overflow-hidden relative">
                        <div className="absolute inset-y-0 left-1/2 w-px bg-muted-fg" />
                        <div 
                ...
                <div className="flex items-center gap-4 text-[10px]">
                    <div>
                        <span className="text-muted-fg">Half-Life:</span>
                        <span className={`ml-1 font-mono font-semibold ${
                            summary.half_life_days && summary.half_life_days < 30 ? 'text-emerald-600' :
                            summary.half_life_days && summary.half_life_days < 60 ? 'text-amber-600' : 'text-foreground/80'
                        }`}>
                            {summary.half_life_days ? `${summary.half_life_days.toFixed(0)}d` : '-'}
                        </span>
                    </div>
                    <div>
                        <span className="text-muted-fg">Vol Ratio:</span>
            ...
            {/* Recommendation */}
            <div className="mt-1.5 text-[10px] text-foreground/80">
...
function ZScoreChart({ data }: { data: RatioAnalysisData }) {
    ...
    return (
        <div className="h-[120px]">
            <div className="flex items-center justify-between px-3 py-1 border-b border-border-subtle">
                <div className="flex items-center gap-2 text-[10px]">
                    <span className="text-foreground/80 font-medium">Z-Score</span>
                    <span className={`font-mono font-semibold ${
                        zscore.current >= 2 ? 'text-rose-600' :
                        zscore.current <= -2 ? 'text-emerald-600' : 'text-foreground/80'
                    }`}>
                        {zscore.current >= 0 ? '+' : ''}{zscore.current.toFixed(2)}
                    </span>
                </div>
            ...
            <ResponsiveContainer width="100%" height="85%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <ReferenceLine y={0} stroke="var(--color-muted-fg)" />
                    <Area
                        type="monotone"
                        dataKey="zscore"
                        stroke="#6366f1"
    ...
    return (
        <div className="flex border-t border-border">
            ...
            <div className="flex-1 p-3 border-r border-border">
                <div className="text-[10px] font-semibold text-foreground/80 mb-2">Position Sizing ($10K)</div>
                <div className="flex items-center gap-4">
                    <div className="text-center">
                        <div className="text-lg font-bold text-rose-600">{hedge.example.shares_y}</div>
                        <div className="text-[9px] text-muted-fg">Buy {data.symbols.y}</div>
                    </div>
                    <ArrowRightLeft className="w-4 h-4 text-muted-fg/50" />
                    <div className="text-center">
                        <div className="text-lg font-bold text-cyan-600">{hedge.example.shares_x}</div>
                        <div className="text-[9px] text-muted-fg">Sell {data.symbols.x}</div>
                    </div>
                </div>
                <div className="mt-2 text-[9px] text-muted-fg">
            ...
            {/* Volatility */}
            {vol && (
                <div className="w-[140px] p-3 border-r border-border bg-surface-hover">
                    <div className="text-[10px] font-semibold text-foreground/80 mb-2">Volatility (Ann.)</div>
            ...
            {backtest && backtest.total_trades > 0 && (
                <div className="w-[160px] p-3 bg-surface-hover">
                    <div className="text-[10px] font-semibold text-foreground/80 mb-2">Backtest (Z=2)</div>
                    <div className="space-y-1">
                        <StatRow 
                            label="Win Rate" 
        ...
        <div className="h-full flex flex-col bg-surface text-foreground" style={{ fontFamily }}>
            ...
            <div className="flex-shrink-0 px-3 py-2 border-b border-border">
                <div className="flex items-center gap-2">
                    ...
                    <ArrowRightLeft className="w-3.5 h-3.5 text-muted-fg/50" />
                            ...
                            <button
                                ...
                                className={`px-2 py-1 text-[10px] font-medium transition-colors ${
                                    period === p.id
                                        ...
                                        : 'text-muted-fg hover:bg-surface-hover'
                                }`}
                            >
                                {p.label}
                            </button>
                    ...
                    <button
                        ...
                        className={`p-1.5 rounded border ${
                            showSettings
                                ...
                                : 'border-border text-muted-fg hover:text-foreground/80'
                        }`}
                    >
                    ...
                    <button
                        ...
                        className="px-3 py-1.5 rounded bg-blue-500 text-white text-[11px] font-medium hover:bg-primary-hover disabled:opacity-50 flex items-center gap-1.5 ml-auto"
                    >
                        {loading ? (
                ...
                {/* Settings row */}
                {showSettings && (
                    <div className="mt-2 pt-2 border-t border-border-subtle flex items-center gap-4 text-[10px]">
                        <div className="flex items-center gap-1">
                            <span className="text-muted-fg">Corr. Window:</span>
                            <input
                                type="number"
                                value={corrWindow}
                                onChange={(e) => setCorrWindow(Math.min(252, Math.max(20, parseInt(e.target.value) || 120)))}
                                className="w-12 px-1 py-0.5 rounded border border-border text-center"
                            />
                            <span className="text-muted-fg">days</span>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 overflow-auto">
                ...
                {!data && !loading && !error && (
                    <div className="h-full flex flex-col items-center justify-center text-muted-fg">
                        ...
                        <p className="text-xs text-muted-fg/50 mt-1">e.g., AAPL vs SPY</p>
                    </div>
                )}
                ...
                {data && !loading && (
                    <div className="divide-y divide-border">
                        ...
                        {/* Summary header */}
                        <div className="px-3 py-2 bg-surface-hover flex items-center justify-between">
                            <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-4 text-[11px]">
                                <div className="flex items-center gap-1">
                                    <span className="text-muted-fg">Beta:</span>
                                    <span className={`font-mono font-semibold ${
                                ...
                                <div className="flex items-center gap-1">
                                    <span className="text-muted-fg">R²:</span>
                                    <span className="font-mono font-semibold text-foreground">
                                        {data.regression?.r_squared.toFixed(2) ?? '-'}
                                    </span>
                                </div>
                                <div className="flex items-center gap-1">
                                    <span className="text-muted-fg">Corr:</span>
            ...
            {/* Footer */}
            <div className="flex-shrink-0 px-3 py-1 border-t border-border bg-surface-hover flex items-center justify-between text-[9px] text-muted-fg">
                <span>
