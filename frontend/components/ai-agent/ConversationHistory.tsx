...
export const ConversationHistory = memo(function ConversationHistory({
  ...
  return (
    <motion.div
      ...
      className="absolute left-0 top-0 bottom-0 w-[280px] bg-surface border-r border-border z-20 flex flex-col shadow-lg"
    >
      {/* Header */}
      <div className="flex-shrink-0 px-3 py-2.5 border-b border-border-subtle flex items-center justify-between">
        <span className="text-[12px] font-semibold text-foreground">Historial</span>
        <button
          onClick={onClose}
          className="text-muted-fg hover:text-foreground/80 transition-colors p-0.5"
        >
      ...
      {/* Search */}
      <div className="flex-shrink-0 px-2.5 py-2 border-b border-border-subtle">
        <div className="relative">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-fg/50" />
          <input
            ref={searchRef}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar..."
            className="w-full pl-7 pr-2 py-1.5 text-[11px] bg-surface-hover border border-border-subtle rounded-lg text-foreground placeholder-muted-fg focus:outline-none focus:border-border transition-colors"
          />
        </div>
      </div>
        ...
        {isLoading && sessions.length === 0 ? (
          <div className="flex items-center justify-center py-8">
            <div className="flex items-center gap-2 text-[10px] text-muted-fg">
              <motion.div
                className="w-3 h-3 border-2 border-border border-t-transparent rounded-full"
                animate={{ rotate: 360 }}
