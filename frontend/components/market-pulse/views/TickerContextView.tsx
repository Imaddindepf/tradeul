...
function MetricRow({ label, value, min, max, diverging, fmt, tag }: {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={`text-[10px] font-bold tabular-nums w-[48px] text-right shrink-0 ${diverging ? vColor(value) : 'text-foreground'}`}>
        {fmt(value)}
      </span>
      {tag && <span className="text-[7px] font-semibold text-muted-fg w-[42px] shrink-0 truncate">{tag}</span>}
    </div>
  );
}
  ...
  return (
    <div className="flex-1 overflow-auto px-3 py-2 space-y-2">
      ...
      <div className="border border-border rounded-md px-3 py-2">
        <div className="flex items-center justify-between mb-1.5">
          ...
          {data.rank_in_industry && (
            <span className="text-[9px] font-semibold text-muted-fg">
        ...
        <div className="grid grid-cols-5 gap-x-3 gap-y-1">
          <div>
            <div className="text-[8px] text-muted-fg font-medium">Price</div>
            <div className="text-[16px] font-bold tabular-nums leading-tight text-foreground">
              ${t.price != null ? (t.price >= 1 ? t.price.toFixed(2) : t.price.toFixed(4)) : '—'}
            </div>
          </div>
          <div>
            <div className="text-[8px] text-muted-fg font-medium">Change</div>
            <div className={`text-[16px] font-bold tabular-nums leading-tight ${vColor(t.change_percent)}`}>
              {pct(t.change_percent)}
            </div>
          </div>
          <div>
            <div className="text-[8px] text-muted-fg font-medium">Volume</div>
            <div className="text-[13px] font-bold tabular-nums leading-tight text-foreground">{fmtNum(t.volume)}</div>
          </div>
          <div>
            <div className="text-[8px] text-muted-fg font-medium">Market Cap</div>
            <div className="text-[13px] font-bold tabular-nums leading-tight text-foreground">{fmtVol(t.market_cap)}</div>
          </div>
          <div>
            <div className="text-[8px] text-muted-fg font-medium">RVOL</div>
            <div className="text-[13px] font-bold tabular-nums leading-tight text-foreground">
      ...
      {data.industry_peers.length > 0 && (
        <div className="border border-border rounded-md px-3 py-2">
          <div className="text-[8px] font-bold text-muted-fg uppercase tracking-[0.1em] mb-1.5">
            Peers — {data.industry}
          </div>
            ...
            {data.industry_peers.map(p => {
              ...
              return (
                <div key={p.symbol} className="flex items-center gap-2 h-[20px] group/peer">
                  ...
                  <span className="text-[8px] text-muted-fg tabular-nums w-[40px] text-right shrink-0">{fmtVol(p.market_cap)}</span>
                </div>
              );
            })}
          </div>
      ...
      {data.themes.length > 0 && (
        <div className="border border-border rounded-md px-3 py-2">
          <div className="text-[8px] font-bold text-muted-fg uppercase tracking-[0.1em] mb-1.5">Themes</div>
          <div className="space-y-[3px]">
            {data.themes.map(th => {
              ...
              return (
                <div key={th.name} className="flex items-center gap-1.5 h-[18px]">
                  ...
                  <span className="text-[8px] text-muted-fg tabular-nums w-[20px] text-right shrink-0">{th.count}</span>
                </div>
              );
            })}
          </div>
