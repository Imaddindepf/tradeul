 ...
 * ejecución simulada (stops/objetivos intrabar, costes, capacidad).
 *
 * Request: {
 ...
 *   date_to:   'YYYY-MM-DD',
 *   execution?: { stop_pct, target_pct, max_hold_min, initial_capital,
 *                 position_size_pct, max_positions, slippage_bps, ... },
 * }
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { strategy_id, date_from, date_to, execution } = body;
    ...
    const res = await fetch(`${BACKTESTER_URL}/api/v1/backtest/triggers/simulate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ strategy, date_from, date_to, execution: execution ?? {} }),
